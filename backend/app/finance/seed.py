...
async def seed_tenant(db, tenant: str = TENANT) -> bool:
    ...
    """
    # Idempotency guard: a re-run must never duplicate ledger history.
    already = (await db.execute(
        select(ChartOfAccount.id).where(ChartOfAccount.tenant_id == tenant).limit(1)
    )).scalar_one_or_none()
    if already is not None:
        print(f"[SKIP] Finance already seeded for {tenant}; leaving existing data untouched.")
        return False
    ...
    acc_checking = ChartOfAccount(id=_id(), tenant_id=tenant, account_code="1010", account_name="SVB Operating Checking", account_type=AccountType.ASSET, normal_balance="DEBIT", is_bank_account=True, currency="USD", is_active=True)
    acc_mmkt = ChartOfAccount(id=_id(), tenant_id=tenant, account_code="1020", account_name="Chase Money Market", account_type=AccountType.ASSET, normal_balance="DEBIT", is_bank_account=True, currency="USD", is_active=True)
    acc_ar = ChartOfAccount(id=_id(), tenant_id=tenant, account_code="1200", account_name="Accounts Receivable (AR)", account_type=AccountType.ASSET, normal_balance="DEBIT", currency="USD", is_active=True)
    acc_ap = ChartOfAccount(id=_id(), tenant_id=tenant, account_code="2000", account_name="Accounts Payable (AP)", account_type=AccountType.LIABILITY, normal_balance="CREDIT", currency="USD", is_active=True)
    acc_equity = ChartOfAccount(id=_id(), tenant_id=tenant, account_code="3000", account_name="Common Stock & Paid-in Capital", account_type=AccountType.EQUITY, normal_balance="CREDIT", currency="USD", is_active=True)
    acc_revenue = ChartOfAccount(id=_id(), tenant_id=tenant, account_code="4000", account_name="SaaS Subscription Revenue", account_type=AccountType.REVENUE, normal_balance="CREDIT", currency="USD", is_active=True)
    acc_infra = ChartOfAccount(id=_id(), tenant_id=tenant, account_code="6010", account_name="Infra & Hosting Expense", account_type=AccountType.EXPENSE, normal_balance="DEBIT", currency="USD", is_active=True)
    acc_ga = ChartOfAccount(id=_id(), tenant_id=tenant, account_code="6020", account_name="Office Supplies & G&A", account_type=AccountType.EXPENSE, normal_balance="DEBIT", currency="USD", is_active=True)
    acc_salaries = ChartOfAccount(id=_id(), tenant_id=tenant, account_code="6100", account_name="Salaries & Wages", account_type=AccountType.EXPENSE, normal_balance="DEBIT", currency="USD", is_active=True)
    ...
    fx_rates = [
        FXRate(id=_id(), tenant_id=tenant, currency="EUR", base_currency="USD", rate=Decimal("1.08000000"), as_of=today - timedelta(days=90)),
        FXRate(id=_id(), tenant_id=tenant, currency="EUR", base_currency="USD", rate=Decimal("1.09200000"), as_of=today - timedelta(days=30)),
        FXRate(id=_id(), tenant_id=tenant, currency="GBP", base_currency="USD", rate=Decimal("1.27100000"), as_of=today - timedelta(days=30)),
        FXRate(id=_id(), tenant_id=tenant, currency="INR", base_currency="USD", rate=Decimal("0.01200000"), as_of=today - timedelta(days=30)),
    ]
    db.add_all(fx_rates)
    ...
    budget_lines = [
        BudgetLine(id=_id(), tenant_id=tenant, budget_id=budget.id, account_id=acc_infra.id, category="Hosting", period=1, period_label="Jan 2026", planned_amount=50000.00, actual_amount=52000.00, committed_amount=0.00, variance=-2000.00),
        BudgetLine(id=_id(), tenant_id=tenant, budget_id=budget.id, account_id=acc_infra.id, category="Hosting", period=2, period_label="Feb 2026", planned_amount=50000.00, actual_amount=48000.00, committed_amount=0.00, variance=2000.00),
        BudgetLine(id=_id(), tenant_id=tenant, budget_id=budget.id, account_id=acc_salaries.id, category="Salaries", period=1, period_label="Jan 2026", planned_amount=100000.00, actual_amount=98000.00, committed_amount=0.00, variance=2000.00),
    ]
    ...
    if _emp is None:
        emp_row = HREmployee(
            id=_id(), tenant_id=tenant, first_name="Jordan", last_name="Rivera",
    ...
    expense_items = [
        ExpenseItem(id=_id(), tenant_id=tenant, report_id=exp_report.id, line_number=1, expense_date=today - timedelta(days=4), category=ExpenseCategory.TRAVEL, description="Flight SF to NYC", merchant="United Airlines", amount=850.00, receipt_path="/receipts/united-850.pdf", is_within_policy=True, is_billable=False),
        ExpenseItem(id=_id(), tenant_id=tenant, report_id=exp_report.id, line_number=2, expense_date=today - timedelta(days=3), category=ExpenseCategory.MEALS, description="Client dinner Stark Ind", merchant="Del Posto", amount=390.00, receipt_path="/receipts/delposto-390.pdf", is_within_policy=True, is_billable=True),
    ]
    ...
    cash_flows = [
        CashFlow(id=_id(), tenant_id=tenant, bank_account_id=bank_svb.id, flow_date=today - timedelta(days=1), flow_type=CashFlowType.OPERATING_INFLOW, category="Customer Subscription", description="Stark Industries monthly subscription", amount=12000.00, currency="USD", is_forecast=False, source_module="AR", fiscal_year=today.year),
        CashFlow(id=_id(), tenant_id=tenant, bank_account_id=bank_svb.id, flow_date=today - timedelta(days=2), flow_type=CashFlowType.OPERATING_OUTFLOW, category="Hosting Bills", description="AWS reserved compute settlement", amount=-8200.00, currency="USD", is_forecast=False, source_module="AP", fiscal_year=today.year),
        CashFlow(id=_id(), tenant_id=tenant, bank_account_id=bank_svb.id, flow_date=today - timedelta(days=5), flow_type=CashFlowType.OPERATING_INFLOW, category="Customer Subscription", description="Wayne Enterprises annual true-up", amount=46000.00, currency="USD", is_forecast=False, source_module="AR", fiscal_year=today.year),
        CashFlow(id=_id(), tenant_id=tenant, bank_account_id=bank_svb.id, flow_date=today - timedelta(days=6), flow_type=CashFlowType.OPERATING_OUTFLOW, category="Payroll Run", description="Semi-monthly payroll disbursement", amount=-70000.00, currency="USD", is_forecast=False, source_module="PAYROLL", fiscal_year=today.year),
        CashFlow(id=_id(), tenant_id=tenant, bank_account_id=bank_svb.id, flow_date=today - timedelta(days=9), flow_type=CashFlowType.OPERATING_OUTFLOW, category="Insurance Premium", description="D&O policy quarterly premium", amount=-5400.00, currency="USD", is_forecast=False, source_module="AP", fiscal_year=today.year),
        CashFlow(id=_id(), tenant_id=tenant, bank_account_id=bank_svb.id, flow_date=today - timedelta(days=12), flow_type=CashFlowType.OPERATING_INFLOW, category="Customer Subscription", description="Mid-market cohort renewals", amount=28500.00, currency="USD", is_forecast=False, source_module="AR", fiscal_year=today.year),
        CashFlow(id=_id(), tenant_id=tenant, bank_account_id=bank_svb.id, flow_date=today - timedelta(days=15), flow_type=CashFlowType.OPERATING_OUTFLOW, category="Tax Payment", description="CA sales tax remittance Q2", amount=-18500.00, currency="USD", is_forecast=False, source_module="TAX", fiscal_year=today.year),
        CashFlow(id=_id(), tenant_id=tenant, bank_account_id=bank_chase.id, flow_date=today - timedelta(days=18), flow_type=CashFlowType.INVESTING_INFLOW, category="Interest Income", description="Money market monthly interest", amount=9100.00, currency="USD", is_forecast=False, source_module="TREASURY", fiscal_year=today.year),
        CashFlow(id=_id(), tenant_id=tenant, bank_account_id=bank_svb.id, flow_date=today + timedelta(days=7), flow_type=CashFlowType.OPERATING_INFLOW, category="Customer Subscription", description="Projected enterprise renewals", amount=95000.00, currency="USD", is_forecast=True, forecast_confidence=0.86, source_module="AR", fiscal_year=today.year),
        CashFlow(id=_id(), tenant_id=tenant, bank_account_id=bank_svb.id, flow_date=today + timedelta(days=12), flow_type=CashFlowType.OPERATING_OUTFLOW, category="Payroll Run", description="Projected semi-monthly payroll", amount=-71000.00, currency="USD", is_forecast=True, forecast_confidence=0.97, source_module="PAYROLL", fiscal_year=today.year),
    ]
    ...
    tax_rules = [
        TaxRule(id=_id(), tenant_id=tenant, name="US Federal Corporate Income Tax", tax_type="INCOME", jurisdiction="US Federal - IRS", rate=0.21, effective_date=date(2018, 1, 1), is_progressive=False, description="Flat corporate rate under the Tax Cuts and Jobs Act; drives the Form 1120 liability estimate.", is_active=True),
        TaxRule(id=_id(), tenant_id=tenant, name="California Sales & Use Tax", tax_type="SALES", jurisdiction="State of California - CDTFA", rate=0.0725, effective_date=date(2017, 7, 1), is_progressive=False, exemptions=["Groceries", "Prescription medicine"], description="Statewide base rate applied to taxable CA sales; district add-ons tracked per invoice.", is_active=True),
        TaxRule(id=_id(), tenant_id=tenant, name="California Corporate Franchise Tax", tax_type="INCOME", jurisdiction="State of California - FTB", rate=0.0884, effective_date=date(2020, 1, 1), is_progressive=False, description="Applies to net income apportioned to California; minimum franchise tax $800.", is_active=True),
    ]
    ...
    db.add(WithholdingConfig(
        id=_id(), tenant_id=tenant, employee_id=_emp, filing_status="SINGLE",
    ...
    report_schedules = [
        ReportSchedule(id=_id(), tenant_id=tenant, name="Monthly P&L to Leadership", report_type=ReportType.INCOME_STATEMENT, frequency="MONTHLY", recipients=["cfo@acme.com", CONTROLLER], format="PDF", is_active=True, last_run_at=now - timedelta(days=14), next_run_at=now + timedelta(days=16), created_by=CONTROLLER),
        ReportSchedule(id=_id(), tenant_id=tenant, name="Weekly Cash Position", report_type=ReportType.CASH_FLOW_STATEMENT, frequency="WEEKLY", recipients=[TREASURY_OPS], format="EXCEL", is_active=True, last_run_at=now - timedelta(days=6), next_run_at=now + timedelta(days=1), created_by=TREASURY_OPS),
    ]
    ...
    db.add(Receipt(
        id=_id(), tenant_id=tenant, customer_id=customers[0].id,
    ...
    for m in months[:3]:
        db.add(FiscalPeriod(
            id=_id(), tenant_id=tenant, fiscal_year=m.year, fiscal_period=m.month,
    ...
    audit_trail = [
        AuditTrail(id=_id(), tenant_id=tenant, event_type="POST", entity_type="JournalEntry", entity_id=je_opening.id, entity_description=f"Opening balances posted as {je_opening.entry_number}", performed_by="SYSTEM", performed_by_type="SYSTEM", timestamp=now - timedelta(days=45)),
        AuditTrail(id=_id(), tenant_id=tenant, event_type="APPROVE", entity_type="Invoice", entity_id=invoices[0].id, entity_description="INV-AWS-2026-05 approved for payment after three-way match", field_changed="status", old_value="PENDING_APPROVAL", new_value="APPROVED", performed_by=CONTROLLER, performed_by_type="USER", timestamp=now - timedelta(days=8)),
        AuditTrail(id=_id(), tenant_id=tenant, event_type="POST", entity_type="Payment", entity_id=payment.id, entity_description=f"Vendor payment {payment.payment_number} recorded against INV-AWS-2026-05", performed_by=TREASURY_OPS, performed_by_type="USER", timestamp=now - timedelta(hours=6)),
        AuditTrail(id=_id(), tenant_id=tenant, event_type="UPDATE", entity_type="SOXControl", entity_id=controls[1].id, entity_description="Vendor Master Setup Controls downgraded after W-9 test exceptions", field_changed="status", old_value="EFFECTIVE", new_value="NEEDS_IMPROVEMENT", performed_by="AI_AGENT", performed_by_type="AI_AGENT", timestamp=now - timedelta(days=13)),
    ]
