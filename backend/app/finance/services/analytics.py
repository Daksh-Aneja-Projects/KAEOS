...
async def finance_analytics(db: AsyncSession, tenant_id: str,
    ...
    # KPI scalars. Overdue invoices are a subset of open AP, so both totals come
    # out of one pass over those rows instead of scanning them twice.
    ...
    ap_q = await db.execute(
        select(sqlfunc.coalesce(sqlfunc.sum(Invoice.balance_due), 0), sqlfunc.count(),
               sqlfunc.coalesce(sqlfunc.sum(case((overdue, 1), else_=0)), 0),
               sqlfunc.coalesce(sqlfunc.sum(case((overdue, Invoice.balance_due), else_=0)), 0))
        .where(Invoice.tenant_id == tenant_id, Invoice.status.in_(OPEN_AP))
    )
    total_ap, open_ap_count, overdue_count, overdue_amount = ap_q.one()
