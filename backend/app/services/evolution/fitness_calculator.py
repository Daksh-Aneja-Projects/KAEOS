...
class FitnessCalculator:
    ...
    async def calculate_fitness(self, tenant_id: str) -> Dict[str, Any]:
        ...
        subscores, factors = _score_graph(nodes, edges, financial_fitness)
...
def _score_graph(nodes: Dict[str, Any], edges: List[Dict[str, Any]],
                 financial_fitness: float) -> tuple[Dict[str, float], Dict[str, Any]]:
    """Score one org-graph snapshot. Returns (subscores, factors).

    Pure: `financial_fitness` is the one figure the graph cannot answer, so
    the caller fetches it and passes it in.
    """
