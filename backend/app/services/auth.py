...
# ── JWT (RFC 7519 via python-jose, already a dependency) ─────────────────────
# Replaces the previous hand-rolled base64(json)+HMAC token. jose enforces the
# `alg` header, so a token cannot be down-graded to alg="none", and gives us
# standard `iss`/`aud`/`exp` handling and constant-time signature verification.
_JWT_ALG = "HS256"
_JWT_ISS = "kaeos"
_JWT_AUD = "kaeos-api"
