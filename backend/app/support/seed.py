...
async def seed():
    ...
    async with AsyncSessionLocal() as db:
        ...
        # crash on the ticket_number unique constraint.
        existing = (await db.execute(
            select(sqlfunc.count()).select_from(Ticket).where(Ticket.tenant_id == TENANT)
        )).scalar() or 0
        if existing:
            print(f"[SKIP] Support already seeded for {TENANT} ({existing} tickets) - skipping.")
            return
        ...
        # 2. Support Agents
        agents = [
            SupportAgent(id=_id(), tenant_id=TENANT, name="Michael Scott", email="michael.scott@kaeos.io", team_id=t1.id, is_ai=False, is_active=True),
            SupportAgent(id=_id(), tenant_id=TENANT, name="Triage Bot", email="triage.bot@kaeos.io", team_id=t1.id, is_ai=True, is_active=True),
            SupportAgent(id=_id(), tenant_id=TENANT, name="Jim Halpert", email="jim.halpert@kaeos.io", team_id=t1.id, is_ai=False, is_active=True),
            SupportAgent(id=_id(), tenant_id=TENANT, name="Pam Beesly", email="pam.beesly@kaeos.io", team_id=t2.id, is_ai=False, is_active=True),
            SupportAgent(id=_id(), tenant_id=TENANT, name="Dwight Schrute", email="dwight.schrute@kaeos.io", team_id=t2.id, is_ai=False, is_active=True),
            SupportAgent(id=_id(), tenant_id=TENANT, name="Holly Flax", email="holly.flax@kaeos.io", team_id=t3.id, is_ai=False, is_active=True),
            SupportAgent(id=_id(), tenant_id=TENANT, name="Toby Flenderson", email="toby.flenderson@kaeos.io", team_id=t3.id, is_ai=False, is_active=False),
        ]
        for a in agents:
            db.add(a)
        await db.flush()
        ...
        channels = [
            SupportChannel(id=_id(), tenant_id=TENANT, channel_name="Email Ingress", channel_type=ChannelType.EMAIL, routing_team_id=t1.id, is_active=True),
            SupportChannel(id=_id(), tenant_id=TENANT, channel_name="Web Portal Widget", channel_type=ChannelType.PORTAL, routing_team_id=t1.id, is_active=True),
            SupportChannel(id=_id(), tenant_id=TENANT, channel_name="Live Chat Widget", channel_type=ChannelType.CHAT, routing_team_id=t2.id, is_active=True),
            SupportChannel(id=_id(), tenant_id=TENANT, channel_name="Billing Phone Line", channel_type=ChannelType.PHONE, routing_team_id=t3.id, is_active=True),
        ]
        for ch in channels:
            db.add(ch)
        await db.flush()
        ...
        for idx, (priority, status, cust, subject, desc, agent, team, channel,
            ...
            tk = Ticket(
                id=_id(), tenant_id=TENANT, ticket_number=f"TCK-{99020 + idx}",
            ...
            db.add(tk)
            tickets.append(tk)
            tags.append(TicketTag(id=_id(), tenant_id=TENANT, ticket_id=tk.id, tag=tag))
        await db.flush()
        for tg in tags:
            db.add(tg)
        ...
        def _c(ticket, author_type, author_id, body, minutes_after, internal="No"):
            return TicketComment(
                id=_id(), tenant_id=TENANT, ticket_id=ticket.id,
        ...
        policies = [
            SLAPolicy(id=_id(), tenant_id=TENANT, name="Standard Response SLA", priority_level="LOW", response_target_mins=240, resolution_target_hrs=48, is_active=True),
            SLAPolicy(id=_id(), tenant_id=TENANT, name="Priority Response SLA", priority_level="MEDIUM", response_target_mins=120, resolution_target_hrs=24, is_active=True),
        ...
        metric = SLAMetric(
            id=_id(), tenant_id=TENANT, date_label=datetime.now(timezone.utc).strftime("%Y-%m-%d"),
        ...
        cat_security = KBCategory(id=_id(), tenant_id=TENANT, name="Account Access & Security", slug="account-security", description="FAQs on credentials and resets")
        cat_billing = KBCategory(id=_id(), tenant_id=TENANT, name="Billing & Invoicing", slug="billing-invoicing", description="FAQs on invoices, refunds, and payment methods")
        db.add(cat_security)
        db.add(cat_billing)
        await db.flush()

        articles = [
            KBArticle(id=_id(), tenant_id=TENANT, category_id=cat_security.id,
            ...
            KBArticle(id=_id(), tenant_id=TENANT, category_id=cat_billing.id,
        ...
        feedback_rows = [
            ArticleFeedback(id=_id(), tenant_id=TENANT, article_id=pw_article.id, is_helpful=True, comment="Fixed it in two minutes, thanks."),
            ArticleFeedback(id=_id(), tenant_id=TENANT, article_id=pw_article.id, is_helpful=True, comment=None),
            ArticleFeedback(id=_id(), tenant_id=TENANT, article_id=sso_article.id, is_helpful=True, comment="Would help to link the exact Okta app template."),
            ArticleFeedback(id=_id(), tenant_id=TENANT, article_id=sso_article.id, is_helpful=False, comment="Our IdP is Azure AD and the steps did not quite match."),
            ArticleFeedback(id=_id(), tenant_id=TENANT, article_id=invoice_article.id, is_helpful=True, comment="Clear breakdown, exactly what I needed."),
        ]
        ...
        csats = [
            CustomerSatisfaction(id=_id(), tenant_id=TENANT, ticket_id=t.id, rating=r, comment=c, sentiment=s,
        ...
        themes = [
            FeedbackTheme(id=_id(), tenant_id=TENANT, theme_name="Fast response on technical questions", volume_percentage=35.50, severity_rating="LOW"),
            FeedbackTheme(id=_id(), tenant_id=TENANT, theme_name="Confusing billing cycle explanations", volume_percentage=22.00, severity_rating="MEDIUM"),
            FeedbackTheme(id=_id(), tenant_id=TENANT, theme_name="Slow report generation on large datasets", volume_percentage=18.30, severity_rating="HIGH"),
            FeedbackTheme(id=_id(), tenant_id=TENANT, theme_name="Positive onboarding experience", volume_percentage=24.20, severity_rating="LOW"),
        ]
        for th in themes:
            db.add(th)
        ...
        rules = [
            EscalationRule(id=_id(), tenant_id=TENANT, rule_name="VIP Ticket SLA Breach",
                           trigger_condition="SLA_BREACH_RESPONSE", escalate_to_team_id=t2.id,
                           time_threshold_mins=15, is_active=True),
            EscalationRule(id=_id(), tenant_id=TENANT, rule_name="Repeated Urgent Contact",
        ...
        escalation = EscalationEvent(
            id=_id(), tenant_id=TENANT, ticket_id=export_stuck.id, rule_id=rules[0].id,
