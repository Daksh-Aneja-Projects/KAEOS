...
| Governed gateway for third-party agents (MCP/A2A) | Enterprise | Partial: MCP callers governed as their own principals with earned rungs, caps and proof trails; typed contract catalog; OAuth2.1 machine-to-machine token acceptance ships (an external agent's own IdP client-credentials token, verified via that IdP's real JWKS, admin-activated per SSO connection; live-unverified - no IdP credentials on this side yet); A2A pending |
