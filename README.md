...
| Governed execution surface (proofs, ladder, rehearsals, external agents, outcomes, committees, quality) | Open core shell, Enterprise data | Available: the surface ships in the open core and shows the capability notice where the Enterprise package is absent. Committees: named approvers cast one ballot each on a paused run (convened from the HITL queue), the pooled verdict resumes or stops it and is sealed as a proof. Quality: thumbs-up / thumbs-down on any sealed run and a pinned regression suite, both read by the trust ladder |
