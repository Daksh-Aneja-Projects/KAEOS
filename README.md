...
| Governed gateway for third-party agents (MCP/A2A) | Enterprise | Partial: MCP and A2A callers both governed as their own principals with earned rungs, caps and proof trails (same forwarder, same gate); typed contract catalog; OAuth2.1 machine-to-machine token acceptance ships (an external agent's own IdP client-credentials token, verified via that IdP's real JWKS, admin-activated per SSO connection; live-unverified - no IdP credentials on this side yet). A2A ships Agent Card discovery + message-to-task exchange against an EXPLICITLY named skill (never free-text intent routing - not invented here); no streaming, push notifications, or a live reference-client interop pass yet |
| Outcome-verified billing with invoice proofs | Enterprise | Partial: outcomes classified from the record, metered unit becomes the verified outcome, invoice export bundles the proofs; per-class Stripe pricing ships (autonomous/assisted each report to their own metered price when configured - the plan's shared allowance still nets against the total first, the resulting overage splits by each class's share of this period's volume; a tenant not on per-class prices is completely unaffected) |
| Governed execution surface (proofs, ladder, rehearsals, external agents, outcomes, committees, quality, evidence) | Open core shell, Enterprise data | Available: the surface ships in the open core and shows the capability notice where the Enterprise package is absent. Committees: named approvers cast one ballot each, on a paused run (convened from the HITL queue) or a standalone question (convened from the tab); the pooled verdict resumes or stops the run and is sealed as a proof. Quality: thumbs-up / thumbs-down on any sealed run and a pinned regression suite, both read by the trust ladder. Evidence: the procurement / AI-Act pack, section by section, and its download. External agents: kill switch plus caps reserved at admission |
...
| Enterprise Ontology: typed object/property/link/interface/value/action types over the data model | Enterprise | Partial: object/property/link/interface/value/action types, four kinds of object set, a generic query+write surface with property-level optimistic concurrency; "mapped" types describe existing tables read-only (zero migration), "native" types get a governed store when none exists; an ActionType delegates to the unchanged department-gate pipeline, never a second execution path |
...
| Dataset Transactions: versioned, branchable sync history over the Ontology | Enterprise | Partial: snapshot/append/update/delete transactions with branch pointers, real isolation for native objects (a what-if branch is provably invisible from "main" and vice versa); does not generalize the per-connector incremental-cursor gap, which stays per-adapter work |
| Object Explorer: a generic UI over the Enterprise Ontology | Open core shell, Enterprise data | Available: browse and edit typed objects (optimistic-concurrency conflicts surfaced, never silently overwritten), apply governed ActionTypes through the same gate pipeline, see markings and masked properties, browse/create Value Types, Interfaces (with a live "verify conformance" action) and Link Types, browse an object's linked objects and create new links, create Object Sets and view resolved members, and view/create Dataset Transaction branches - the surface ships in the open core and shows the capability notice where the Enterprise package is absent |
