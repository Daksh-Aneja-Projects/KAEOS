...
import {
 FlaskConical, Satellite, Users, Gauge, Loader2, Zap, ScrollText,
 Share2, Code2, Database, AlertTriangle, CheckCircle, TrendingUp,
...
const SIGNAL_TYPES = ['REGULATORY', 'VENDOR', 'MARKET', 'THREAT', 'ECONOMIC'];
...
type Lane = 'intelligence' | 'organization' | 'simulation' | 'benchmark' | 'ledgers';

const LANES: { key: Lane; label: string; icon: React.ElementType }[] = [
...
const riskColor = (level: string | undefined, colors: any) =>
...
const cleanReason = (s: string | null | undefined) =>
...
const PioneerLab = () => {
 ...
 const primaryBtn = (busy: boolean): React.CSSProperties => ({
  background: colors.primary, color: '#fff', opacity: busy ? 0.6 : 1,
 });
 ...
 const ErrorBanner = ({ message }: { message: string | null }) => message ? (
  <div className="px-4 py-3 rounded-lg text-[12px]" style={{ background: `${colors.error}15`, color: colors.error }}>
   {message}
  </div>
 ) : null;

 const ActionButton = ({ busy, onClick, disabled, children }: {
  ...
  <button
   ...
   style={{ ...primaryBtn(busy), opacity: disabled ? 0.4 : busy ? 0.6 : 1 }}
  >
