 ...
 return (
  <div className="h-full overflow-y-auto" style={{ background: colors.canvas, color: colors.ink }}>
   <div className={`${PAGE_PAD} space-y-5`}>
    ...
    <div className="flex gap-2" role="tablist" aria-label="Extraction sections"
      onKeyDown={e => {
       if (e.key !== 'ArrowRight' && e.key !== 'ArrowLeft') return;
       e.preventDefault();
       const next = tab === 'signals' ? 'candidates' : 'signals';
       setTab(next);
       document.getElementById(`extraction-tab-${next}`)?.focus();
      }}>
     {(['signals', 'candidates'] as const).map(t => {
      const active = tab === t;
      return (
       <button key={t} id={`extraction-tab-${t}`} onClick={() => setTab(t)}
       ...
       <div key={s.id} style={card} className="p-4">
        <div className="flex items-center justify-between mb-2 gap-2 flex-wrap">
         <div className="flex items-center gap-2 flex-wrap">
          <span className="px-2 py-0.5 text-[11px] rounded" style={{ background: colors.info + '18', color: colors.info, border: `1px solid ${colors.info}30` }}>{humanize(s.source_type)}</span>
          <span className="px-2 py-0.5 text-[11px] rounded" style={{ background: colors.surface2, color: colors.inkSubtle }}>{humanize(s.domain)}</span>
          {s.pii_present && (
      ...
      {candidates.length === 0 ? (
       ...
       <div key={c.id} style={card} className="p-4">
        <div className="flex items-center gap-2 mb-2">
         ...
         <span className="px-2 py-0.5 text-[11px] rounded" style={{ background: colors.primary + '18', color: colors.primary, border: `1px solid ${colors.primary}30` }}>{humanize(c.domain)}</span>
        </div>
