...
| **Governance & Provenance** | Every recommendation is explainable, traceable, and auditable - an append-only, hash-chained provenance ledger (unification onto one verifiable scheme is in progress - see Known Limitations), fairness audits, red-team checks, and blocking HITL gates. Explainable by design, not a black box | `/provenance`, `/10x/quantum-events`, `/fairness/audit-log`, `/redteam`, `/hitl` |
