...
| **Provenance** | Hash-chained, append-only `ProvenanceLedger` for every AI decision with full lineage; explainable by design. Honest boundary: chain verification is being unified onto one signed scheme; until then treat it as append-only evidence, not independently verifiable |
